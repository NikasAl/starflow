...
@export var owner_id: int = Constants.PlayerId.NONE : set = set_owner_id
@export var level: int = 1 : set = set_level
@export var max_level: int = 5
@export var production_rate: float = Constants.BASE_PRODUCTION_RATE
@export var planet_type: int = Constants.PlanetType.NORMAL
...
func _check_upgrade_or_capture(attacker_id: int) -> void:
        if pending_ships >= Constants.PLANET_CAPTURE_THRESHOLD:
...
func _update_visuals() -> void:
        ...
        if mat:
                mat.albedo_color = Constants.PLAYER_COLORS.get(owner_id, Color.WHITE)
