...
func _process(delta: float) -> void:
	...
	for planet: Planet3D in GameManager.all_planets:
...
func _spawn_ship_for_planet(planet: Planet3D) -> void:
	# Проверяем наличие активного исходящего потока
	var streams: Array[ShipStream3D] = GameManager.stream_manager.get_streams_from(planet)
	if streams.size() > 0:
...
func _cleanup_stale_accumulators() -> void:
	...
	for key: Planet3D in _production_accumulators:
		if not is_instance_valid(key):
			stale_keys.append(key)
	for key: Planet3D in stale_keys:
