...
var all_planets: Array[Dictionary] = []  ## {planet: Planet3D, owner: int, level: int, production: float}
var all_streams: Array[Dictionary] = []  ## {source: Planet3D, target: Planet3D, owner: int, count: int}
var distances: Dictionary = {}            ## PackedStringArray key -> float distance
var threat_assessments: Dictionary = {}   ## Planet3D -> float threat level
var own_planets: Array[Planet3D] = []
var enemy_planets: Array[Planet3D] = []
var neutral_planets: Array[Planet3D] = []

func capture_snapshot(ai_player_id: int, planets: Array[Planet3D], streams: Array[ShipStream3D]) -> void:
	all_planets.clear()
	own_planets.clear()
	enemy_planets.clear()
